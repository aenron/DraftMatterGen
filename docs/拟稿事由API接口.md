...
SUMMARY_SINGLE_FILE_MAX_CHARS=40
SUMMARY_MULTIPLE_TOTAL_CHARS=90
```
...
服务会先完成文件校验和文本解析，按可进入模型摘要的文件数预先计算字数预算，并把正文上限写入模型提示词。单文件的摘要正文提示上限为 40 个字符；多个可摘要文件时，摘要正文与文件间全角分号合计的提示预算为 90 个字符，每份正文上限为 `floor(SUMMARY_MULTIPLE_TOTAL_CHARS / 可摘要文件数)`，非最后一份预留一个全角分号。服务端追加的最后固定尾句“。文件需要盖章。”共 8 个字符，不计入上述字数预算。服务不会对模型超出提示上限的输出做硬截断，以保持原文完整；前面的成功文件以全角分号结尾，最后一个成功文件以“文件需要盖章。”结尾。长文档只使用开头最多 4,000 字符生成一次摘要；若前部识别到疑似目录，会将目录与开头内容一并输入模型，不再进行多切片摘要或二次汇总。模型调用受全局 `LLM_MAX_CONCURRENCY` 控制，摘要输出上限可通过 `SUMMARY_LLM_MAX_TOKENS` 调整。
